...
size_long = {'pa': (alen,), 'pb': (blen,), 'pc': (clen,), 'pd': (dlen,), 'pc_a': (clen, a.size), 'pc_b': (clen, b.size),
             'pc_d': (clen, d.size), 'pc_ab': (clen, a.size, b.size), 'pc_abd': (clen, a.size, b.size, d.size),
             'pb_a': (blen, a.size),
...
def test_distribution(f, model, arg_long, arg_short, size_long, size_short):
    ...
    if var.shape != (size_long[0],):
        raise Exception('Function %s for model %d returns distribution support of the wrong size: %s instead of %s.' % (
        f, model, str(var.shape), str((size_long[0],))))
    if prob.shape != size_long:
        raise Exception(
            'Function %s for model %d returns distribution probabilities of the wrong size: %s instead of %s.' % (
            f, model, str(prob.shape), str(size_long)))
    ...
    if prob.shape != size_short:
        raise Exception(
            'Function %s for model %d returns distribution probabilities of the wrong size: %s instead of %s.' % (
            f, model, str(prob.shape), str(size_short)))
    if t > max_time:
...
def test_generate(f, model, arg_long, arg_short, size_long, size_short):
    ...
    if gen.shape != size_long:
        raise Exception(
            'Function %s for model %d returns distribution probabilities of the wrong size: %s instead of %s.' % (
            f, model, str(gen.shape), str(size_long)))
    ...
    if gen.shape != size_short:
        raise Exception(
            'Function %s for model %d returns distribution probabilities of the wrong size: %s instead of %s.' % (
            f, model, str(gen.shape), str(size_short)))
    if t > max_time:
        print('Warning: function %s for model %d is too slow: %.4f seconds.' % (f, model, t))
    print('%s, model %d: \t ok; \t time: %.4f seconds' % (f, model, t))
    return
